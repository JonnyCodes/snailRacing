...
export class Snail extends Loadable {
    ...
    get config(): ISnailConfig {
        return this._config;
    }

    constructor(snailConfig: ISnailConfig) {
        super();

        this._config = snailConfig;
        this._speed = randomIntBetween(Snail.MIN_SPEED, Snail.MAX_SPEED);
        this._currentMaxSpeed = Snail.MAX_SPEED - (this._speed >= (Snail.MAX_SPEED + Snail.MIN_SPEED) / 2 ? this._speed - ((Snail.MAX_SPEED + Snail.MIN_SPEED) / 2) : 0);
